# Trainiert Modelle mit versch. Patience und max_steps Einstellungen. 
# Soll die Auswirkungen der Trainingsdauer untersuchen 
...
runs = 30          # Anzahl zu trainierender Modelle, pro Trainingsdauer
numberOfModels = 3 # Anzahl an verschiedenen Trainingsdauerkonfigurationen

anzTrainData = 28 # Größe des Trainingsdatensatzes
anzEvalData  = 12 # Größe des Evaluierungs- und Entwicklungsdatensatzes (development data)
...
def exportCurrentStatusModellOverview(overview, name):
    ...
    fileName = './TestTrainDauer/Results_{}.jsonl'.format(name)
...
for i in range(numberOfModels):   
    # zählt, wie lange das Programm pro Trainingsdauerkonfiguration braucht
    ...
    # trainiert und evaluiert auf jeder Trainingsdauerkonfiguration runs-viele Modelle
    for run in range(runs):
        ...
        train("./TestTrainDauer/config_{}.cfg".format(i), "./TestTrainDauer/TempModell")
        durationTrain = datetime.datetime.now() - durationTrain
        modell = spacy.load("./TestTrainDauer/TempModell/model-best/")
        ...
        modelOverviews.append(generateOverview(modell, evaluationScores, anzTrainData, anzEvalData, anzEvalData, str(durationTrain), str(durationEval), str(durationModel), run, trainIndexes, devIndexes, evalIndexes))
    ...
    exportFinishedModellOverview(modelOverviews, "Länge_{}".format(i))
